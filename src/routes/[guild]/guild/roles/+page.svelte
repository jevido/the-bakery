...
<div class="px-7 py-[22px]">
	<div class="mb-[18px]">
		<div class="font-heading font-bold text-[23px]">Roles</div>
				...
				<div class="bg-[var(--card)] border border-[var(--line)] rounded-[13px] overflow-hidden mb-[10px]">
					...
					{#each g.perms as p}
						...
						<div class="flex items-center gap-[13px] px-[18px] py-3 border-b border-b-[var(--line)]">
							<div class="w-[34px] h-5 rounded-[10px] shrink-0 relative {has ? 'bg-[var(--grn)]' : 'bg-white/[0.08]'} {selectedRole.master ? 'cursor-not-allowed' : 'cursor-pointer'}">
								<div class="size-[14px] rounded-full bg-white absolute top-[3px] transition-[left] duration-150" style:left={has ? '17px' : '3px'}></div>
							</div>
							<div class="flex-1 min-w-0">
