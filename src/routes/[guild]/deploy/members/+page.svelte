<script lang="ts">
	...
	const canManage = $derived(
		myRole ? (roleAc[myRole as keyof typeof roleAc]?.authorize({ guild: ['manage_members'] }).success ?? false) : false
	);
...
<div class="px-7 py-[22px]">
	<div class="flex items-center justify-between mb-[18px]">
		<div>
			...
			<div class="text-[13px] text-[var(--tx-2)] mt-0.5">{data.members.length} members in this guild</div>
		</div>
		{#if canManage}
			<button
	...
	<div class="bg-[var(--card)] border border-[var(--line)] rounded-[13px] overflow-hidden">
		<div class="grid grid-cols-[2fr_1.3fr_1fr_140px] gap-[14px] px-[18px] py-[10px] border-b border-b-[var(--line)] text-[11px] font-bold tracking-[.05em] text-[var(--tx-3)]">
			<div>MEMBER</div>
			<div>ROLE</div>
			<div>JOINED</div>
			<div></div>
		...
		{#each data.members as m (m.id)}
			...
			<div class="grid grid-cols-[2fr_1.3fr_1fr_140px] gap-[14px] items-center px-[18px] py-[13px] border-b border-b-[var(--line)]">
				<!-- Avatar + name -->
				<div class="flex items-center gap-3">
					<div class="size-9 rounded-[9px] flex items-center justify-center font-heading font-bold text-[14px] shrink-0" style:background={av.bg} style:color={av.fg}>{initials(m.user.name)}</div>
					<div>
				...
				<!-- Role badge -->
				<div>
					<span class="text-[12.5px] font-semibold px-[10px] py-1 rounded-[20px]" style:background="{rm.color}22" style:color={rm.color}>{rm.name}</span>
				</div>
